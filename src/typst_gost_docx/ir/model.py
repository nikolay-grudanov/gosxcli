...
class InlineLinkNode(BaseNode):
    """Inline hyperlink to an external URL.

    Generated by ``#link("url")[label]`` or plain ``#link("url")`` in Typst.
    The writer emits a real ``<w:hyperlink r:id="...">`` element so the
    URL survives round-tripping through Microsoft Word.
    """

    node_type: NodeType = NodeType.INLINE_LINK
    url: str = ""
    text: str = ""
...
class ValidationIssue(BaseModel):
    """A single validation problem with full source attribution.

    Carries the offending label, the kind of issue (undefined reference,
    unreferenced label, missing citation), and the source location so the
    CLI can produce actionable error messages like
    ``test.typ:42: Undefined reference: @fig:missing``.
    """

    label: str
    kind: str  # one of "undefined_ref", "unreferenced_label", "missing_citation"
    file_path: str = ""
    line: int = 0
    column: int = 0
...
class ValidationResult(BaseModel):
    ...
    Attributes:
        ...
        issues: Полный список проблем с source-location, если валидатор
            собирал детальную информацию. ``undefined_refs`` и
            ``unreferenced_labels`` синхронизируются с этим списком для
    ...
    def format_report(self) -> str:
        ...
        Uses ``issues`` (with source locations) when available, falling
        back to ``undefined_refs`` / ``unreferenced_labels`` plain sets
        for older callers.
        ...
        # Prefer the rich issue list when populated.
        if self.issues:
            undefined = [i for i in self.issues if i.kind == "undefined_ref"]
            unreferenced = [i for i in self.issues if i.kind == "unreferenced_label"]
            missing_cit = [i for i in self.issues if i.kind == "missing_citation"]
        else:
            undefined = [
                ValidationIssue(label=ref, kind="undefined_ref")
                for ref in sorted(self.undefined_refs)
            ]
            unreferenced = [
                ValidationIssue(label=lbl, kind="unreferenced_label")
                for lbl in sorted(self.unreferenced_labels)
            ]
            missing_cit = []

        if undefined:
            lines.append("Undefined references (errors):")
            for issue in sorted(undefined, key=lambda i: (i.file_path, i.line, i.label)):
                loc = self._format_location(issue)
                prefix = f"{loc}: " if loc else ""
                lines.append(f"  WARNING: {prefix}@{issue.label}")
            lines.append("")

        if missing_cit:
            lines.append("Missing bibliography entries (errors):")
            for issue in sorted(missing_cit, key=lambda i: (i.file_path, i.line, i.label)):
                loc = self._format_location(issue)
                prefix = f"{loc}: " if loc else ""
                lines.append(f"  WARNING: {prefix}@[{issue.label}]")
            lines.append("")
        ...
        if unreferenced:
            ...
            for issue in sorted(unreferenced, key=lambda i: (i.file_path, i.line, i.label)):
                loc = self._format_location(issue)
                prefix = f"{loc}: " if loc else ""
                lines.append(f"  INFO: {prefix}<{issue.label}>")
            lines.append("")

        # Если нет проблем
        if not undefined and not unreferenced and not missing_cit:
    ...
    def _format_location(issue: ValidationIssue) -> str:
        """Render ``file_path:line`` (or just ``file_path``) for a location."""
